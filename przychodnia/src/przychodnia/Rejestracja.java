...
public class Rejestracja extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        //button
        Button button1 = new Button("Załóż konto");
        button1.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent e) {
                try {
                    pst = connect_baza.getConnection().prepareStatement("INSERT INTO pacjenci (imie, nazwisko, PESEL, adres, telefon, email, haslo) VALUES (?,?,?,?,?,?,MD5(?))");
                    pst.setString(1, nameInput.getText());
                    pst.setString(2, nazwiskoInput.getText());
                    pst.setInt(3, nrPInput.getAnchor());
                    pst.setString(4, adresInput.getText());
                    pst.setInt(5,telInput.getAnchor());
                    pst.setString(6, mailInput.getText());
                    pst.setString(7, passInput.getText());
                    pst.execute();
                   Alert alert = new Alert(AlertType.INFORMATION);
                  alert.setTitle("Informacja");
                  alert.setHeaderText(null);
                  alert.setContentText("Uzutkownik dodany");
                  alert.showAndWait();
                    
                
pst.close();
                } catch (SQLException ex) {
                    AlertRej alt =new AlertRej();
                    alt.start(window);
